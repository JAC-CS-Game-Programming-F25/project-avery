...
export default class Camera {
  ...
  constructor(player, viewportWidth, viewportHeight, worldWidth, worldHeight) {
    ...
    this.verticalDeadzone = viewportHeight / 0.25;
  }
  ...
  update(dt) {
    ...
    const groundedBias = this.player.isOnGround
      ? CameraSettings.verticalBias
      : 0;

    let targetY;

    if (this.player.isOnGround) {
      // Grounded: biased, stable camera
      targetY =
        this.lastGroundedY - this.center.y - CameraSettings.verticalBias;
    } else {
      // Airborne: follow the player directly
      targetY = playerCenter.y - this.center.y;
    }
  ...
  startShake(intensity = 6, duration = 0.25) {
